...
#define BOOST_TEST_MODULE nViewsTriangulationLORansac
...
// is considered).
BOOST_AUTO_TEST_CASE(LORansacTriangulate_Algebric)
{
  ...
  for(std::size_t trial = 0; trial < numTrials; ++ trial)
  {
    ...
    using TriangulationKernel = aliceVision::LORansacTriangulationKernel<>;
    TriangulationKernel kernel(pt2d, Ps);
    robustEstimation::ScoreEvaluator<TriangulationKernel> scorer(threshold);
    Vec4 X = robustEstimation::LO_RANSAC(kernel, scorer, &vec_inliers);
